...
const EditPostPage = () => {
  ...
  const handleSubmit = async () => {
    if (!title || !subject || !postContent || editor.isEmpty) {
    ...
    try {
      ...
      const res = await fetch(`/api/blogs/${id}`, {
        ...
        body: JSON.stringify({ title, subject, post: postContent, id: session.user.id }),
      });
  ...
  if (status === "loading" || loading) return <SubtleSpinner />;
